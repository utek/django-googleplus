...
import utils
from models import GooglePlusUser
from utils import get_registration_backend
...
REDIRECT_URI = urlparse.urljoin(
        ...
        'http://local.tank24.org:8000', GOOGLEPLUS_LOGIN_URL
)
...
def _create_or_update_googleplus_user(request, profile, access_token, expires_in, backend):
    ...
    try:
        ...
        print "uzytkownik nie istnieje. Tworzenie"
        size = 9
        ...
        user = User.objects.create(first_name=first_name, last_name=last_name, username='googleplus_' + profile['id'])
    ...
    if user_is_created:
        googleplus_user = user.get_profile()
    ...
    googleplus_user.googleplus_access_token = access_token
    googleplus_user.googleplus_expiry_at = datetime.datetime.now() + \
